...
def load_pipeline(args):
    ...
    if args.use_gt_mask:
        # Inpainting pipeline, but ONLY canny + depth ControlNet
...
def process_image_inpaint(pipe, depth_processor, depth_model, depth_device, input_path, output_path, args, guidance_scale):
    """GT-mask-driven inpainting path, conditioned on the mask plus the
    canny and depth ControlNets."""
    image = Image.open(input_path).convert("RGB")
